...
#define TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) stl::detail::is_trivially_copy_constructible<T>::value
...
// TODO SFINAE
template <class T>
struct hash<m5::stl::optional<T>> {
    ::std::size_t operator()(const m5::stl::optional<T> &o) const {
        if (!o.has_value()) return 0;

        return std::hash<m5::stl::detail::remove_const_t<T>>()(*o);
    }
};
}  // namespace std
